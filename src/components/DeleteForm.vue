...
function handleForm() {
    let form = document.getElementById('delete-form').getElementsByClassName("text-input");
    let fields = [], values = []
...
function delete_row(event){
    let fields = event.target.getElementsByClassName("text-input");
    let id = fields[0].value
    sql_query.s_query = `DELETE FROM ${selected_table.value} WHERE id = ${id};`
    emit('executeQuery')
}
...
<template>
    <form id="delete-form" class="filled-form" @submit.prevent="delete_row">
        <fieldset>
            ...
            <br />
            <button id="submit-btn" type="submit" >Видалити запис</button>
        </fieldset>
    </form>
</template> 